...
export const assemble_config = (config) => {
  ...
  for (const sub_definition of def) {
    ...
    for (const object_from_config_to_find_id_for of config[src_object_name]) {
      ...
      const objects_to_find = object_from_config_to_find_id_for[property_with_ids_from_src_object]

      for (const object of objects_to_find) {
        const found = config[property_with_ids_from_src_object].find(o => o.id === object.id)
        const found_index = config[property_with_ids_from_src_object].findIndex(o => o.id === object.id)
