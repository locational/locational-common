...
export const assemble_config = (config) => {
  ...
  for (const sub_definition of def) {
    ...
    for (const object_from_config_to_find_id_for of config[src_object_name]) {
      ...
      const ids_to_find = cloneDeep(object_from_config_to_find_id_for[property_with_ids_from_src_object])
      object_from_config_to_find_id_for[property_with_ids_from_src_object] = []

      for (const id of ids_to_find) {
        const found = config[property_with_ids_from_src_object].find(o => o.id === id)
        console.log('found', found)
        object_from_config_to_find_id_for[property_with_ids_from_src_object].push(found)
      }
    }
  }
